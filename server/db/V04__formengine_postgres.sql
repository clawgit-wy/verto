-- =====================================================================
-- 表单引擎与流程仿真测试工作台 - PostgreSQL 初始化脚本
-- 对应设计：form-engine/design-proposal.md
-- 适用数据库：PostgreSQL 12+（本机 Docker 实例 infra-pool-postgres / 库 jeecg_boot）
-- 说明：从 MySQL 迁移而来，已去除 MySQL 专属语法（ENGINE/AUTO_INCREMENT/反引号等），
...
CREATE TABLE IF NOT EXISTS fe_process (
    ...
    version       VARCHAR(50),
    status        VARCHAR(20)  DEFAULT 'draft',   -- draft / imported / simulated / exported
    form_schema   TEXT,                            -- form-create 设计器产出的 JSON Schema
    process_def   TEXT,                            -- 导入的流程定义 JSON（节点+表单绑定）
...
COMMENT ON TABLE  fe_process IS '表单引擎-流程列表';
COMMENT ON COLUMN fe_process.status IS 'draft=草稿 imported=已导入 simulated=已仿真 exported=已导出';
...
COMMENT ON TABLE fe_form_template IS '表单引擎-表单模板';
...
CREATE TABLE IF NOT EXISTS fe_form_field (
    ...
    field_type    VARCHAR(50),                     -- input/number/select/button...
    default_value VARCHAR(500),
    permissions   TEXT,                            -- JSON: {"applicant":"write","manager":"readonly","finance":"readonly"}
    sort_no       INTEGER      DEFAULT 0
);
COMMENT ON TABLE fe_form_field IS '表单引擎-字段元数据与权限矩阵';
...
CREATE TABLE IF NOT EXISTS fe_form_method (
    ...
    params        TEXT,                            -- JSON 数组，入参字段 key 列表
    description   VARCHAR(500)
);
COMMENT ON TABLE fe_form_method IS '表单引擎-可绑定业务方法';
...
-- 表单模板
INSERT INTO fe_form_template (id, template_name, version, description) VALUES
...
INSERT INTO fe_form_field (id, template_id, field_key, field_label, field_type, default_value, permissions, sort_no) VALUES
...
-- 业务方法
INSERT INTO fe_form_method (id, method_key, method_name, params, description) VALUES
...
INSERT INTO fe_process (id, process_name, process_code, template_id, template_name, version, status, form_schema, process_def, update_time) VALUES
...
--       MySQL 迁移至 PostgreSQL 后需建空表，否则 FlowExecutor 初始化失败
...
COMMENT ON TABLE airag_flow IS 'AI流编排规则表（LiteFlow SQL规则源；迁移PG时新建空表，未启用AI流功能）';
